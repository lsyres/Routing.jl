...
function solveESPPRC(org_pg::ESPPRC_Instance; max_neg_cost_routes=Inf, method="bidirectional")
    if method == "pulse"
        return solveESPPRCpulse(org_pg; max_neg_cost_routes=max_neg_cost_routes)
    elseif method == "monodirectional"
        return monodirectional(org_pg; max_neg_cost_routes=max_neg_cost_routes)
    else
        return bidirectional(org_pg; max_neg_cost_routes=max_neg_cost_routes)
    end
end
