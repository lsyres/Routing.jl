...
Graph::Graph(std::vector<std::vector<int>>& data, std::vector<double>& dual_var) {
    ...
    for (int i = 0; i < node_number; i++) {
        for (int j = 0; j < node_number; j++) {
            ...
            dist = std::floor(10*dist)/10; // as done in solomon literature 
            // std::cout << "(" << i << "," << j << "): dist=" << dist << ", dv=" << dual_var[i] << std::endl;
            if (node_list[i].twl + node_list[i].service_time + dist > node_list[j].twu)
                continue;
            edge_list.push_back(Edge(i, j, dist - dual_var[i], dist, head, index));
        }
    }
    outgoing_edge_sort();
}
